...
export default function StatistiquesClient({ products }: { products: Product[] }) {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8">
      ...
      {products.length === 0 ? (
        <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-border-2 py-16 text-center">
          <svg viewBox="0 0 24 24" className="size-10 text-muted" fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
            <path d="M3 17l4-8 4 4 4-6 4 4" />
            <path d="M3 21h18" />
          </svg>
          <p className="font-semibold text-fg">Aucune statistique pour le moment</p>
          <p className="max-w-xs text-sm text-muted">
            Référencez votre premier produit pour voir vos statistiques apparaître ici.
          </p>
          <Link
            href="/dashboard/produits/nouveau"
            className="mt-2 inline-flex items-center gap-2 rounded-xl bg-accent px-4 py-2.5 text-sm font-semibold text-accent-fg shadow-soft transition-colors hover:bg-accent-hover"
          >
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3 sm:gap-4">
            {[
              ...
              { label: "Clics sortants", value: totalClicks.toLocaleString("fr-FR"), sub: "redirections vers votre plateforme" },
