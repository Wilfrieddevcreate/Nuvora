  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8">
          ...
          <div className="space-y-3">
            {recentProducts.map((p) => (
              ...
              >
                {/* Mini cover */}
                <div
                  className={`flex size-12 shrink-0 items-center justify-center rounded-xl bg-linear-to-br ${COVER_COLOR[p.category] ?? "from-surface-2 to-surface"}`}
                >
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-border bg-surface p-5 sm:flex-row sm:items-center sm:gap-6 shadow-soft">
        ...
        <Link
          href={`/createur/${user?.slug ?? "profil"}`}
